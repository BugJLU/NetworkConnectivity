...
class NetworkGraph(object):
    def __init__(self, vertices = 0):
        ...
        self.nodes = list(range(vertices))
        self.edges = []
        self.adjMat = [[]]
    ...
    def addEdges(self, edges):
        for i in edges:
            self.edges.append(i)
            self.adjMat[edges[i][0]][edges[i][1]] += 1
            # TODO: check
    ...
    def generateComponent(self):
        # component = {i: [i] for i in self.nodes}
        component = [self.nodes[0]]
        index = 0
        while len(component) > index:
            for i, val in enumerate(self.edges[component[index]]):
                if val > 0 and i not in component:
                    component.append(i)
            index += 1
        return component
